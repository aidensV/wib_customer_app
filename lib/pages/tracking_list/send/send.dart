...
class SendNota extends StatefulWidget {
  SendNota({Key key, this.title}) : super(key: key);
  final String title;
  @override
  State<StatefulWidget> createState() {
    return _SendNotaState();
  }
}
...
class _SendNotaState extends State<SendNota> {
  Future<List<ListNota>> listNotaAndroid() async {
    ...
    try {
      ...
      final nota = await http.post(url('api/TransaksiPengirimanAndroid'),
          headers: requestHeaders,
          body: {
            'urutkan': _urutkantransaksidelivered,
            'tanggal_awal': _tanggalawaldelivered.toString(),
            'tanggal_akhir': _tanggalakhirdelivered.toString(),
          });
  ...
  @override
  void initState() {
    _scaffoldKeydelivered = new GlobalKey<ScaffoldState>();
  ...
  _scaffoldKeydelivered.currentState
                  ...
                  ? RefreshIndicator(
                      ...
                      child: Column(children: <Widget>[
                        ...
                        Padding(
                          ...
                          child: Center(
                            child: Text("Anda tidak memiliki transaksi pengiriman",
        ...
        onTap: () async {
          dynamic filter = await Navigator.push(
            ...
            MaterialPageRoute(
              ...
              builder: (BuildContext context) => FilterTransaksidelivered(
                urutkantransaksidelivered: _urutkantransaksidelivered,
                tanggalawaldelivered: _tanggalawaldelivered,
                tanggalakhirdelivered: _tanggalakhirdelivered,
              ),
            ),
          );
          if (filter != null) {
            _urutkantransaksidelivered = filter['_urutkantransaksidelivered'];
            _tanggalawaldelivered = filter['_tanggalawaldelivered'];
            _tanggalakhirdelivered = filter['_tanggalakhirdelivered'];
            setState(() {
              _urutkantransaksidelivered = filter['_urutkantransaksidelivered'];
              _tanggalawaldelivered = filter['_tanggalawaldelivered'];
              _tanggalakhirdelivered = filter['_tanggalakhirdelivered'];
